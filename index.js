...
const template = require("./component_template/template_ts");
...
function createReactApp() {
  return new Promise((resolve) => {
    if (appName) {
      ...
      try {
        shell.exec(
          `node ${require("path").dirname(
            require.main.filename
          )}/node_modules/create-react-app/index.js ${appName}`,
          (e, stdout, stderr) => {
            if (stderr) {
              if (e == 127) {
                console.log(
                  `create-react-app not installed \n install create-react-app first globally use :`
                    .red
                );
...
function installPackages() {
  return new Promise((resolve) => {
    console.log(
      "\nInstalling react-router, react-router-dom, react-lazy-route, axios, sass and redux..."
        .cyan
    );
    shell.exec(
      `npm install --save react-router react-router-dom react-lazy-route axios mobx mobx-react node-sass-chokidar npm-run-all`,
      { cwd: appDirectory },
      (e) => {
...
function updatePackage_json() {
  ...
  return new Promise((resolve) => {
    ...
    shell.exec(
      `node ${require("path").dirname(
        require.main.filename
      )}/node_modules/json/lib/json.js -I -f package.json -e 'this.scripts=` +
...
function generateBoilerplate() {
  ...
  return new Promise((resolve) => {
    ...
    fs.copySync(
      `${require("path").dirname(require.main.filename)}/templates`,
      `${appDirectory}/src`
    );
    resolve();
  });
...
async function createModule(module, cmd) {
  newCompPath = module;
  ...
  if (fs.existsSync("./src/pages")) {
    newCompPath = `./src/pages/${module}`;
  }
  let template = await buildTemplate();
  writeFile(template, module, module, "tsx", true, false);
  let reducer = await buildReducer();
  const provider = await buildProvider();
  writeFile(reducer, module, "state/reducer", "ts", false, true);
  writeFile("", module, "state/actions", "ts", false, true);
  writeFile(provider, module, "state/Provider", "tsx", false, true);
}
function buildReducer() {
  let reducer = [template.reducer].join("\n");
  return reducer;
}
function buildProvider() {
  let provider = [template.provider].join("\n");
  return provider;
}
...
async function createComponent(component, cmd) {
  ...
  let template = await buildTemplate();
  writeFile(template, component, "tsx", true);
}
function buildTemplate() {
  ...
  let exported = observable
    ? [template.exported.observable]
...
function writeFile(template, component, file, type, capitalized, folder) {
  ...
  if (path && !capitalized && !folder) {
    path = path + "/" + comp;
  } else if (path && !capitalized && folder) {
    path = path + "/" + file;
  } else if (path) {
  ...
  if (!fs.existsSync(`${path}.${type}`)) {
    fs.outputFile(`${path}.${type}`, template, (err) => {
      if (err) throw err;
      replace({
        regex: ":className",
      ...
      console.log(`Component ${comp} created at ${path}.${type}`.cyan);
    });
  } else {
    console.log(
      `Component ${comp} allready exists at ${path}.${type}, choose another name if you want to create a new component`
        .red
    );
  }
}
